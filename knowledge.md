...
- **Analyzer decodes must be capped** (Android): `computeChannelLevels`/`computeWaveform` in PlayerScreen and the Visualizer's peak/RMS passes decode to f32le then `readBytes()` — a full-length 5.1 decode is ~1.15 MB/s (6ch f32le @ 48 kHz, ~0.19 MB/s per channel) and will OOM (`OutOfMemoryError` is an `Error`, not caught by `catch(Exception)`). Always pass `-t 60` (or stream) to bound the decode. The visualizer's levels/peaks now reflect only the first 60 s of long tracks by design.
